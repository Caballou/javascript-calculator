...
function App() {
  ...
  const numArray = ['7','8','9','4','5','6','1','2','3','0','00','.']
  const numIdArray = ['seven','eight','nine','four','five','six','one','two','three','zero','double-zero','decimal']
  ...
  const addInput = (val) => {
    ...
    } else { /*Si la operación no terminó (todavía es String)...*/
        ...
        <div className='div-botones'>
          ...
          <div className='div-mem'>
            {memArray.map((_,index) => (
              <Boton power={power} key={index}>{memArray[index]}</Boton>
            ))}
            
            </div>

          <div className='div-clear'>
            <Boton power={power} onClick={powerOffOn}>AC</Boton>
            <Boton power={power} onClick={changeSing}>±</Boton>
            <Boton power={power} onClick={backSpace}>⌫</Boton>
            <Boton id='clear' power={power} onClick={clearScreen}>C</Boton>
          </div>
          
          <div className='div-numeros'>
          {numArray.map((_,index) => (
              <Boton id={numIdArray[index]} key={index} input={input} onClick={addInput}>
                {numArray[index]}
              </Boton>
            ))}
          </div>
          ...
          {ops1Array.map((_,index) => (
              <Boton id={opsIdArray[index]} key={index} power={power}
              onClick={addInput}>
                {ops1Array[index]}
              </Boton>
            ))}
          ...
          <div className='div-mas'>
            <Boton id='add' onClick={addInput}>+</Boton>
          </div>

          <div className='div-ops2'>
            <Boton id='subtract' onClick={addInput}>-</Boton>
            <Boton id='equals' onClick={showResult}>=</Boton>
          </div>
